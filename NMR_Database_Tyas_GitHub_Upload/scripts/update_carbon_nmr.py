...
def update_carbon_nmr():
    try:
        conn = sqlite3.connect(path_database)
        cursor = conn.cursor()

        print("\n=== Update Data Carbon NMR ===")

        peak_id = input_integer("Masukkan ID peak karbon     : ")

        cursor.execute("""
            SELECT id, compound_id, delta_ppm, carbon_type,
        ...
        if not result:
            print("\nID peak karbon tidak ditemukan.")
            conn.close()
            return
        ...
        print(f"solvent              : {result[5]}")
        print(f"instrument_mhz       : {result[6]}")
        print(f"note                 : {result[7]}")
        ...
        print("\nBerhasil! Data carbon NMR telah diperbarui.")
        ...
        print(f"Terjadi error saat update data carbon NMR: {e}")
