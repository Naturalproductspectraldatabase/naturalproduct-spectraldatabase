...
def cari_kemiripan_1h():
    try:
        conn = sqlite3.connect(path_database)
        cursor = conn.cursor()

        print("\n=== Pencarian Berdasarkan 1H NMR ===")
        print("Masukkan daftar peak 1H dipisah koma.")
        print("Contoh: 4.93, 4.83, 4.67, 2.23, 1.72")
        ...
        tolerance_input = input("Toleransi ppm [default 0.05]: ").strip()
        if tolerance_input == "":
            tolerance = 0.05
        else:
            try:
                ...
                print("Input toleransi tidak valid. Dipakai default 0.05 ppm.")
                tolerance = 0.05
        ...
        for compound in compounds:
            ...
            # Ambil semua peak 1H senyawa ini
            cursor.execute("""
                SELECT delta_ppm
                FROM proton_nmr
                WHERE compound_id = ?
        ...
        for i, item in enumerate(hasil[:10], start=1):
            ...
            print(f"Jumlah peak 1H DB    : {item['db_peak_count']}")
        ...
        print(f"Terjadi error saat pencarian 1H: {e}")
